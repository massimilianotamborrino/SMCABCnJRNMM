                             ...
                             attempt, folder, whichprior = 'unif',subsamplingby=1)
{
  ...
  nfreepar_d <- N*(N-1)# % number of discrete parameters (which are N(N-1))
  ...
  RES<-foreach(success = 1:numparticles,.combine='rbind',.packages=c('SMCABCnJRNMM')) %dopar% {
    ...
    return(list(numproposals,distance, c(parameters[1:nfreepar_c],Rho_vec[-which(Rho_vec==Inf)])))#    return(list(numproposals,distance,parameters,simsumm_all))
  }
  eval_time <- toc()
...
#  simsumm_all<-matrix(as.numeric(unlist(RES[,4])),nrow=nsummaries)
  ...
  write.table(distance_accepted,file=sprintf('%s/distance_stage%d_attempt%d.txt',folder,t,attempt),row.names = FALSE,col.names = FALSE)
...
ABCthreshold_new<- ABCthreshold
  ...
  hat_p_vec<-sapply(1:nfreepar_d,FUN=function(I) sum(ABCdraws_d[I,])/numparticles)
  ...
  while (acceptrate>minaccept.rate) {#(totnumproposals < number_sim){ # I stop when the acceptance rate goes below a given minimum amount
    t <- t+1
    {if(acceptrate< minaccept.rate*10){ABCthreshold_new <- quantile(distance_accepted,alpha/100);
    }
  else ABCthreshold_new <- median(distance_accepted);}
...
#   simsumm_all<- c();
      while(distance>=ABCthreshold_new & numproposals<number_sim){
        ...
        numproposals <- numproposals +1;
          ...
          Rhom<-perturb_discrete(N,hat_p_vec,stay_prob)
     ...
     if(dens==0) return(print('error'))
      Rho_vec<-as.vector(t(Rhom))
      list(numproposals,distance,theta_c,Rho_vec[-which(Rho_vec==Inf)],prior/dens,numproposals0,numproposalsneg)#  list(numproposals,distance,theta_c,Rhov,simsumm_all,prior/dens,numproposals0,numproposalsneg)
    }
    eval_time <- toc()
...
#'@rdname SMCABCnJRNMM_acceptrate2
#'@title SMCABCnJRNMM_acceptrate2
#'@description SMC-ABC for time-varying thresholds computed as percentiles of ACCEPTED distances with a fixed budget (number of simulations)
#'@param data observed data
#'@param Pr_cont Matrix of lower/upper bounds for the uniform priors for the continuous parameters
#'@param stay_prob Probability of not perturbing the sampled discrete connection probability
...
#'@export
#'
SMCABCnJRNMM_acceptrate2<- function (data, Pr_cont,stay_prob,extra_model, ABCthreshold, minaccept.rate, numparticles, alpha, sampling,
                                    attempt, folder, whichprior = 'unif',subsamplingby=1)
{
  N<-extra_model[[1]]
  T<-extra_model[[2]]
...
#'@rdname SMCABCnJRNMM_acceptrate_variation
#'@title SMCABCnJRNMM_acceptrate_variation
#'@description SMC-ABC for time-varying thresholds computed as percentiles of ACCEPTED distances with a fixed budget (number of simulations)
#'@param data observed data
#'@param Pr_cont Matrix of lower/upper bounds for the uniform priors for the continuous parameters
#'@param stay_prob Probability of not perturbing the sampled discrete connection probability
...
#'@export
#'
SMCABCnJRNMM_acceptrate_variation<- function (data, Pr_cont,stay_prob,extra_model, ABCthreshold, minaccept.rate, numparticles, alpha, sampling,
                                    attempt, folder, whichprior = 'unif',subsamplingby=1)
{
  N<-extra_model[[1]]
  T<-extra_model[[2]]
